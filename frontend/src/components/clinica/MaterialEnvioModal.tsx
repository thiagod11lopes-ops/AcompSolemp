    ...
    >
      <Box
        sx={(theme) => ({
          px: 3,
          py: 2.5,
          background: `linear-gradient(135deg, ${theme.palette.secondary.dark} 0%, ${theme.palette.secondary.main} 55%, ${theme.palette.info.main} 100%)`,
          color: 'white',
          display: 'flex',
        ...
        <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center' }}>
          ...
          <Box>
            ...
            <Typography variant="body2" sx={{ opacity: 0.9, mt: 0.25 }}>
              {consumoRows.length} lançamento(s) selecionado(s) · Revise e gere a planilha no formato MODELO
            </Typography>
          </Box>
        </Box>
        <IconButton onClick={onClose} disabled={isGenerating} sx={{ color: 'white' }} aria-label="Fechar">
          <CloseIcon />
        </IconButton>
      </Box>
      ...
      <Box sx={{ flex: 1, overflow: 'auto', p: 3, display: 'grid', gap: 2.5 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
          ...
          <Button
            ...
            disabled={isGenerating || consumoRows.length === 0}
          >
            {isGenerating ? 'Gerando planilha...' : 'Gerar planilha .ods'}
          </Button>
        </Box>
        {exportError && (
        ...
        <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, bgcolor: 'background.default' }}>
          ...
          >
            {IMH_ASSINATURAS.slice(0, 2).map((label) => (
      ...
      >
        ...
        >
          {isGenerating ? 'Gerando...' : 'Gerar planilha .ods'}
        </Button>
      </Box>
    </Dialog>
  )
