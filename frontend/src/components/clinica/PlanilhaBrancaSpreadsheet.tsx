...
function PlanilhaBrancaSpreadsheetInner({
  ...
  const cells = useMemo(
    () => normalizeSpreadsheetGrid(grid, PLANILHA_BRANCA_ROWS, PLANILHA_BRANCA_COLS),
    [grid],
  )
  ...
  const colLetters = useMemo(
    () => Array.from({ length: PLANILHA_BRANCA_COLS }, (_, i) => colIndexToLetter(i)),
    [],
  )
      ...
      <Box sx={{ overflow: 'auto', maxHeight: 'min(70vh, 720px)' }}>
        <Box
          ...
          sx={{
            ...
            tableLayout: 'fixed',
            minWidth: PLANILHA_BRANCA_ROW_HEADER_WIDTH + PLANILHA_BRANCA_COLS * PLANILHA_BRANCA_CELL_WIDTH,
          }}
        >
          <thead>
            <tr>
